...
def pil_unwrap(pil_img):
    ...
    pic_array = (np.array(pil_img.getdata()).reshape((pil_img.size[1], pil_img.size[0], 4)) / 255.0)
...
def _shear_x_impl(pil_img, level):
    ...
    if random.random() > 0.5:
        level = -level
    return pil_img.transform((pil_img.size[0], pil_img.size[1]), Image.AFFINE, (1, level, 0, 0, 1, 0))
...
def _shear_y_impl(pil_img, level):
    ...
    if random.random() > 0.5:
        level = -level
    return pil_img.transform((pil_img.size[0], pil_img.size[1]), Image.AFFINE, (1, 0, 0, level, 1, 0))
...
def _translate_x_impl(pil_img, level):
    ...
    if random.random() > 0.5:
        level = -level
    return pil_img.transform((pil_img.size[0], pil_img.size[1]), Image.AFFINE, (1, 0, level, 0, 1, 0))
...
def _translate_y_impl(pil_img, level):
    ...
    if random.random() > 0.5:
        level = -level
    return pil_img.transform((pil_img.size[0], pil_img.size[1]), Image.AFFINE, (1, 0, 0, 0, 1, level))
